...
def build_state():
    ...
    s[0, 0] = spec[0]["Last1_chunk_bitrate_u"]        # highest bitrate
    # buffer in its own bounds
    s[1, 0] = random.uniform(spec["Last1_buffer_size_l"],
                              spec["Last1_buffer_size_u"])
    s[2, :] = spec[0][[f"Last{i}_throughput"   for i in range(8, 0, -1)]].values
    s[3, :] = spec[0][[f"Last{i}_downloadtime" for i in range(8, 0, -1)]].values
    s[4, :6] = spec[0][[f"chunksize{i}"        for i in range(1, 7)]].values
    s[5, 0] = spec[0]["Chunks_left"]
