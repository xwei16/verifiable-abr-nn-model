...
# -----------------------------
# Remove all compiled vnnlib files
# -----------------------------
def clean_up(vnnlib_dir):
    for fname in os.listdir(vnnlib_dir):
        if fname.endswith(".vnnlib.compiled"):
            path = os.path.join(vnnlib_dir, fname)
            os.remove(path)
            print(f"[x] Removed {fname}")
